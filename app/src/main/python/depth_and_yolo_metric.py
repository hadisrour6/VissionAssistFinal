...
# depth_estimator = pipeline("depth-estimation", model="depth-anything/Depth-Anything-V2-Metric-VKITTI-Small")
depth_estimator = pipeline("depth-estimation", model="depth-anything/Depth-Anything-V2-Metric-Outdoor-Small-hf")

def get_depth_map(frame):
    """
    Convert a cv2 frame (BGR) to a PIL image, run the depth estimator,
    and return a NumPy array with depth values in meters.
    """
...
def get_object_depth(depth_map, box_coords, method="median"):
    ...
    Parameters:
        depth_map (np.array): 2D array of depth values (in meters).
...
def detect_and_section(frame, depth_map):
    ...
    both detection and depth (in meters) info onto the frame.
    """
    ...
    for result in results[0].boxes:
        ...
        if conf > 0.75:
            ...
            # *** CHANGED: Display depth in meters.
            depth_text = f"Depth: {object_depth:.2f} m" if object_depth is not None else "Depth: N/A"
...
while cap.isOpened():
    ...
    if frame_count % frame_interval == 0:
        # Get depth map for the current frame (now in meters)
        depth_map = get_depth_map(frame)
        
        # Run detection and overlay both object and depth information
        frame_with_detections = detect_and_section(frame, depth_map)
        
        # Show and write the processed frame
        cv2.imshow('YOLOv8 Detection with Metric Depth', frame_with_detections)
