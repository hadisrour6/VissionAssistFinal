...
# Function to assign proximity levels based on relative depth
def proximity_level(relative_depth):
    if relative_depth < -1.0:  # Much smaller than the background (very close)
        return "Very Close"
    elif relative_depth < 0.5:  # Slightly smaller or near background depth
        return "Close"
    else:  # Larger depth than the background
        return "Far"
...
while cap.isOpened():
    ...
    if frame_count % frame_interval == 0:
        ...
        # Compute the average depth of the background
        print(f"Calculating background depth for frame {processed_frame_count}...")
        mask = np.ones(depth_image.shape, dtype=bool)  # Create a mask for the entire image
        ...
        for result in results[0].boxes:
            ...
            mask[startY:endY, startX:endX] = False  # Exclude object regions from the mask

        # Calculate the average depth of the background
        background_depth = np.mean(depth_image[mask])
        print(f"Background depth: {background_depth:.2f}")
        ...
        for result in results[0].boxes:
            ...
            if conf > 0.60:
                ...
                if cropped_depth.size > 0:
                    ...
                    object_depth = np.mean(cropped_depth)
                    # Calculate the relative depth compared to the background
                    relative_depth = object_depth - background_depth
                    # Determine proximity level
                    proximity = proximity_level(relative_depth)
                else:
                    object_depth = 0
                    relative_depth = 0
                    proximity = "Far"
                ...
                label = f"{category_index[cls]}: {proximity} ({object_depth:.2f})"
                cv2.putText(frame, label, (startX, startY - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
                print(f"Detected {category_index[cls]} at [{startX}, {startY}, {endX}, {endY}] "
                      f"with confidence {conf:.2f}, object depth {object_depth:.2f}, "
                      f"relative depth {relative_depth:.2f}, proximity: {proximity}")
