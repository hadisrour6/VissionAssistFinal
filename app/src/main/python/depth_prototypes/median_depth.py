...
# Function to assign proximity levels based on median depth
def proximity_level(median_depth):
    if median_depth < 1.0:  # Very close (in meters or arbitrary units)
        return "Very Close"
    elif median_depth < 3.0:  # Close
        return "Close"
    else:  # Far
        return "Far"
        ...
        for result in results[0].boxes:
            ...
            if conf > 0.60:
                ...
                if cropped_depth.size > 0:
                    ...
                    proximity = proximity_level(median_depth)
                else:
                    median_depth = 0  # Default depth if no valid region
                    proximity = "Far"
                ...
                label = f"{category_index[cls]}: {proximity} ({median_depth:.2f})"
                cv2.putText(frame, label, (startX, startY - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
                print(f"Detected {category_index[cls]} at [{startX}, {startY}, {endX}, {endY}] "
                      f"with confidence {conf:.2f}, median depth {median_depth:.2f}, proximity: {proximity}")
