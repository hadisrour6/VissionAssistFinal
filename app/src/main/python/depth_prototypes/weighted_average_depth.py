...
# Function to assign proximity levels based on weighted depth
def proximity_level(weighted_depth):
    if weighted_depth < 1.0:  # Very close (in meters or arbitrary units)
        return "Very Close"
    elif weighted_depth < 3.0:  # Close
        return "Close"
    else:  # Far
        return "Far"
        ...
        for result in results[0].boxes:
            ...
            if conf > 0.60:
                ...
                if cropped_depth.size > 0:
                    # Compute weights (inverse of depth)
                    weights = 1 / (cropped_depth + 1e-6)  # Add a small constant to avoid division by zero
                    # Compute weighted average depth
                    weighted_average_depth = np.sum(cropped_depth * weights) / np.sum(weights)
                    
                    # Determine proximity level
                    proximity = proximity_level(weighted_average_depth)
                else:
                    weighted_average_depth = 0  # Default depth if no valid region
                    proximity = "Far"
                ...
                label = f"{category_index[cls]}: {proximity} ({weighted_average_depth:.2f})"
                cv2.putText(frame, label, (startX, startY - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
                print(f"Detected {category_index[cls]} at [{startX}, {startY}, {endX}, {endY}] "
                      f"with confidence {conf:.2f}, weighted depth {weighted_average_depth:.2f}, proximity: {proximity}")
