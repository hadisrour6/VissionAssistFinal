...
temp_image_folder = os.path.join(base_dir, "temp_images")  # Save segmented depth images here
...
while cap.isOpened():
    ...
    if frame_count % frame_interval == 0:
        ...
        # Compute percentile-based min and max depth for reference
        ...
        # Counter for unique cropped images per frame
        object_counter = 0
        ...
        for result in results[0].boxes:
            ...
            # Process only if confidence level is high
            if conf > 0.60:
                ...
                if cropped_depth.size > 0:
                    # Find the most dominant depth value (object should be at similar depth)
                    median_depth = np.median(cropped_depth)

                    # Define depth range (adaptive filter to remove background)
                    depth_range = 0.1 * median_depth  # Keep values within ±10% of median

                    # Create a mask: Keep only depth values close to the object's median depth
                    depth_mask = (cropped_depth >= (median_depth - depth_range)) & \
                                 (cropped_depth <= (median_depth + depth_range))

                    # Apply mask
                    filtered_depth_values = cropped_depth[depth_mask]

                    if filtered_depth_values.size > 0:
                        average_depth = np.mean(filtered_depth_values)  # Average only valid pixels
                    else:
                        average_depth = 0  # If no valid depth data remains

                    # Save segmented object depth
                    segmented_object = np.zeros_like(cropped_depth, dtype=np.uint8)
                    segmented_object[depth_mask] = cropped_depth[depth_mask]  # Keep only object depth

                    object_name = category_index[cls] if cls in category_index else "unknown"
                    segmented_filename = f"segmented_{object_name}_{processed_frame_count}_{object_counter}.png"
                    segmented_path = os.path.join(temp_image_folder, segmented_filename)
                    cv2.imwrite(segmented_path, segmented_object)
                    print(f"Saved segmented depth object: {segmented_path}")

                    object_counter += 1  # Increment counter for unique names

                else:
                    average_depth = 0  # Default if no valid depth data
                ...
                # Add label and depth information (raw depth value)
                label = f"{category_index[cls]}: Depth {average_depth:.2f}"
