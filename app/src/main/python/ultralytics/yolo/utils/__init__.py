...
def get_user_config_dir(sub_dir='Ultralytics'):
    ...
    # If a custom directory is specified via environment variables, use it.
    env_config = os.environ.get("YOLO_CONFIG_DIR") or os.environ.get("ULTRALYTICS_HOME")
    if env_config:
        ...
        if os_name == 'Windows':
            ...
            # On Android, Chaquopy sets HOME to a writable directory.
            path = Path(os.environ.get("HOME", Path.home())) / sub_dir
        else:
            raise ValueError(f'Unsupported operating system: {os_name}')

    # Verify that the parent directory is writable.
    # (Assumes is_dir_writeable is defined elsewhere in your code.)
    if not is_dir_writeable(str(path.parent)):
        raise Exception(
            f"Parent directory {path.parent} is not writable. "
            "Please set YOLO_CONFIG_DIR or ULTRALYTICS_HOME to a writable location."
        )
